...
export default function AdminDashboard() {
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 bg-white text-black">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-black tracking-tight">Enterprise Overview</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {metrics.map((metric) => (
                    <div key={metric.label} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all group text-black">
                        ...
                        <p className="text-[10px] uppercase tracking-[0.1em] text-gray-500 font-bold mb-1">{metric.label}</p>
                        <h3 className="text-2xl font-bold tracking-tight text-black">{metric.value}</h3>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Recent Orders List */}
                <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden text-black">
                    <div className="p-6 border-b border-gray-100 flex items-center justify-between">
                        <h3 className="font-bold text-xs uppercase tracking-widest text-gray-500">Latest Transactions</h3>
                        <Link href="/admin/orders" className="text-xs font-bold text-red-600 hover:underline">View Ledger</Link>
                    </div>
                    <div className="divide-y divide-gray-50">
                        {stats?.recentOrders?.length > 0 ? stats.recentOrders.map((order: any) => (
                            <div key={order.id} className="p-5 hover:bg-gray-50/50 transition-colors flex items-center justify-between group">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-gray-50 flex items-center justify-center border border-gray-200 text-gray-500 group-hover:text-red-600 group-hover:border-red-100 transition-colors">
                                        <ShoppingBag className="w-5 h-5" />
                                    </div>
                                    <div>
                                        <p className="font-bold text-sm text-black">Order #{order.id.slice(-6).toUpperCase()}</p>
                                        <p className="text-[10px] text-gray-500 uppercase tracking-wide">{order.items?.[0]?.name || 'Internal Transaction'}</p>
                                    </div>
                                </div>
                            ...
                            <div className="p-12 text-center text-gray-500 text-xs font-medium">No recent orders found.</div>
                        )}
                    </div>
                </div>

                {/* Growth Chart / Info */}
                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col justify-between text-black">
                    <div>
                        <h3 className="font-bold text-xs uppercase tracking-widest text-gray-500 mb-8">Stock Logistics</h3>
